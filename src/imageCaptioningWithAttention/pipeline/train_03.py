...
class TrainPipeline():
    def __init__(self, train_dataset, val_dataset):
        ...
        self.model, self.criterion = build_model(self.train_config.vocab_size, self.train_config.hidden_dim, self.train_config.pad_token_id, self.train_config.max_length, self.train_config.nheads, self.train_config.enc_layers, self.train_config.dec_layers, self.train_config.dim_feed_forward, self.train_config.dropout, self.train_config.lr_backbone, self.train_config.backbone, self.train_config.dilation, self.train_config.pre_norm, self.device)
    ...
    def predict(self, image_path, model_name):
        ...
        output = self.trainer.evaluate_caption(self.model, image, caption, caption_mask, self.train_config.max_length)
